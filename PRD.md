...
- **Belum** (bagian berikutnya): buka laci manual tanpa transaksi (wajib dicatat + PIN opsional, §19.2), printer Bluetooth Klasik/BLE, USB, printer bawaan all-in-one (Sunmi, iMin) & fallback printer sistem, Wizard Uji Perangkat & `Perangkat.ProfilHardware`, struk digital `/s/{kodeStruk}` + QR di struk, struk retur/void/tutup shift/pre-order, tiket dapur per stasiun.
